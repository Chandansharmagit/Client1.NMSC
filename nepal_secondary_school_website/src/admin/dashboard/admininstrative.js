...
const Admin_contact = () => {
    ...
    const fetchImages = async () => {
        try {
            const response = await axios.get('https://backend.nepalmodelsecondaryschool.com/admin');
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            setLoading(true);
            const response = await axios.post('https://backend.nepalmodelsecondaryschool.com/admin', formDataToSend, {
                headers: {
    ...
    const handleDelete = async (imageId) => {
        try {
            await axios.delete(`https://backend.nepalmodelsecondaryschool.com/admin/${imageId}`);
            fetchImages();
        } catch (error) {
    ...
    return (
        <div className="container">
            <h2>For administrative staff Details</h2>
            <form onSubmit={handleSubmit}>
                ...
                <input type="text" name="title3" value={formData.title3} onChange={handleInputChange} placeholder="Contact Number" />
